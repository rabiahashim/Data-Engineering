...
def get_complains_data():
    #complains_data_url=st.secrets["private_sheet_url"]
    complains_data_url="https://docs.google.com/spreadsheets/d/e/2PACX-1vT6F25oYggRRBwr_ZKMwNFH7pAxGF39nVyD7yX637bIZO9wO9Vjr4IWKq9HFL9zSg/pubhtml?gid=395055207&single=true"
    dfs=pd.read_html(complains_data_url,header=1)
    df_complain=pd.DataFrame([s.to_dict() for s in dfs])
...
with st.container():
    ...
    #col5.selectbox( 'Select the State',options=df_complain['state'].unique())
